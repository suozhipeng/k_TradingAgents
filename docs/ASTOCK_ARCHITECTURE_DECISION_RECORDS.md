...
本文记录 TradingAgents-Astock 后续开发中的关键架构和产品决策，避免在 Phase 30-38 中反复争议。本文只记录当前核心功能范围内的决策，不展开安全与隐私、SLA 与故障分级、用户角色/RBAC。
...
| 关联文档 | `ASTOCK_REQUIREMENTS.md`, `ASTOCK_WEBUI_PRODUCT_SPEC.md` |
...
| 关联文档 | `ASTOCK_API_CONTRACTS.md`, `ASTOCK_LIVE_TRADING_RUNBOOK.md`, `ASTOCK_RISK_DISCLOSURE_AND_COMPLIANCE.md` |
...
| 关联文档 | `ASTOCK_PRODUCT_OPTIMIZATION_ROADMAP.md`, `ASTOCK_STRATEGY_DEVELOPMENT_GUIDE.md` |
...
| 关联文档 | `ASTOCK_MODEL_GOVERNANCE.md`, `ASTOCK_RISK_DISCLOSURE_AND_COMPLIANCE.md` |
...
| 关联文档 | `ASTOCK_WEBUI_PRODUCT_SPEC.md`, `ASTOCK_WEBUI_PAGE_ACCEPTANCE_CHECKLIST.md` |
...
| 关联文档 | `ASTOCK_DATA_MIGRATION_AND_UPGRADE.md`, `ASTOCK_RELEASE_AND_CHANGE_MANAGEMENT.md` |
...
| 决策 | 每个 phase 的真实交付证据以 `docs/phases/phase-XX-*.md` 为准，当前状态集中写入 `ASTOCK_CURRENT_STATUS.md` |
| 影响 | 后续开发必须补需求 ID、scope、测试、风险、下一入口条件和 commit SHA |
| 关联文档 | `README.md`, `ASTOCK_REQUIREMENTS_TRACEABILITY_MATRIX.md` |

## 3. 新 ADR 规则

- 涉及 core、API 契约、数据 schema、交易能力等级、WebUI 顶层导航、AI 输出边界的重大变更必须新增 ADR。
- ADR 不替代需求文档；需求仍进入 `ASTOCK_REQUIREMENTS.md`、`ASTOCK_BACKLOG.md` 和追踪矩阵。
