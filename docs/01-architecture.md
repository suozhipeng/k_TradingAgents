...
| 决策 | 每个 phase 的真实交付证据以 `docs/phase-archive.md` 为准，当前状态集中写入 `docs/phase-archive.md` |
| 影响 | 后续开发必须补需求 ID、scope、测试、风险、下一入口条件和 commit SHA |
| 关联文档 | `docs/phase-archive.md`, `04-development.md` |

## 3. 新 ADR 规则

- 涉及 core、API 契约、数据 schema、交易能力等级、WebUI 顶层导航、AI 输出边界的重大变更必须新增 ADR。
- ADR 不替代需求文档；需求仍进入 `04-development.md`、`BACKLOG.md` 和追踪矩阵。
...
本文梳理 TradingAgents-Astock 当前后台 API、所属模块、数据源、能力等级、真实/模拟边界和测试验收。`01-architecture.md` 定义 API 规范；本文列出现有与 Phase 30-39 目标 API 清单。
...
- 新增 endpoint 必须同步本文和 `01-architecture.md`。
- 数据源变化必须同步 `03-operations.md`。
- schema 变化必须同步 `04-development.md`。
