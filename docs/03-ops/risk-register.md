...
| R-002 | QMT 订单、成交、撤单、拒单和券商回报 reconciliation 未闭环 | 交易 | H | M | Phase 30/35 先补订单生命周期、reconciliation、风控前置门和审计引用；真实券商 reconciliation 明确 P3 暂不处理 | `BACKLOG.md`, `04-dev/traceability-matrix.md`, `docs/phases/phase-35-trading-execution-control.md` | accepted（P3 暂不处理，产品定位非实盘） |
...
| R-009 | Phase 文档与真实代码状态漂移，导致后续开发依据不可靠 | 项目管理 | M | M | 已完成多次文档批量同步，当前状态已对齐；后续 phase 变更后需同步维护 | `docs/README.md`, `04-dev/traceability-matrix.md` | closed（文档已对齐，后续维护模式下持续） |
