...
class QMTAdapter(AStockAdapterBase):
    """Keep QMT as a read-only placeholder; execution is explicitly out of scope."""

    name = "qmt"

    def _hint(self, request: AStockRequest, method: str):
        return self._unavailable(request, "QMT bridge remains read-only placeholder in this stage: {0}".format(method))
